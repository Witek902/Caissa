...
DEFINE_PARAM(TM_NodesCountScale, 217, 160, 260);
DEFINE_PARAM(TM_NodesCountOffset, 271, 200, 250);
DEFINE_PARAM(TM_NodesCountMinFactor, 55, 20, 80);
...
void UpdateTimeManager(const TimeManagerUpdateData& data, SearchLimits& limits, TimeManagerState& state)
{
    ...
    // decrease time if nodes fraction spent on best move is high
    {
        ...
        const double minFactor = static_cast<double>(TM_NodesCountMinFactor) / 100.0;
        const double nodeCountFactor = std::max(offset - data.bestMoveNodeFraction * scale, minFactor);
