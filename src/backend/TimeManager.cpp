...
void UpdateTimeManager(const TimeManagerUpdateData& data, SearchLimits& limits, TimeManagerState& state)
{
    ...
    // decrease time if eval is around +1.0 or -1.0
    {
        ...
        const double a = 0.1f; // strength
        const double b = 4.0f; // steepness
        ...
        std::cout << "info string eval=" << data.currResult[0].score << " normalizedEval=" << normalizedEval << " evalFactor=" << evalFactor << std::endl;
    }
