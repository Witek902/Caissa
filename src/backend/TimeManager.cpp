...
void InitTimeManager(const Game& game, const TimeManagerInitData& data, SearchLimits& limits)
{
    ...
    if (data.remainingTime != INT32_MAX)
    {
        ...
        // the soft limit is left alone - reserving time there taxes every move instead of only the
        // ones that could actually flag
        // at a low clock this fraction binds instead of the overhead itself, making it the parameter
        // that governs the trade between losing on time and losing a late iteration
        ...
        const float hardTimeLimit = std::max(minMoveTime,
            std::min(timeMargin * remainingTime, remainingTime - overheadReserve));
