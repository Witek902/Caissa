...
DEFINE_PARAM(TM_MovesLeftMidpoint, 44);
DEFINE_PARAM(TM_MovesLeftSteepness, 22);
DEFINE_PARAM(TM_IdealTimeFactor, 88);
DEFINE_PARAM(TM_NodesCountScale, 205);
DEFINE_PARAM(TM_NodesCountOffset, 49);
...
static float EstimateMovesLeft(const uint32_t moves)
{
    ...
    const float steepness = static_cast<float>(TM_MovesLeftSteepness) / 10.0f;
...
void TimeManager::Init(const Game& game, const TimeManagerInitData& data, SearchLimits& limits)
{
    ...
    {
        const float idealTimeFactor = static_cast<float>(TM_IdealTimeFactor) / 100.0f;
