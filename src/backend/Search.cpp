...
ScoreType Search::NegaMax(ThreadData& thread, NodeInfo* node, SearchContext& ctx)
{
    ...
    while (movePicker.PickMove(*node, move, moveScore))
    {
        ...
        if constexpr (!isRootNode)
        {
            ...
            Prefetch(&corrHist->pawnStructure[stm][position.GetPawnsHash() % PawnCorrTableSize]);
            Prefetch(&corrHist->nonPawnWhite[stm][position.GetNonPawnsHash(White) % NonPawnCorrTableSize]);
            Prefetch(&corrHist->nonPawnBlack[stm][position.GetNonPawnsHash(Black) % NonPawnCorrTableSize]);
            Prefetch(&corrHist->continuation[stm][move.PieceTo()][node->previousMove.PieceTo()]);
            //if (node->ply >= 3 && (node - 2)->previousMove.IsValid())
            //    Prefetch(&corrHist->continuation[stm][move.PieceTo()][(node - 2)->previousMove.PieceTo()]);
        }
