...
void Search::ThreadData::UpdateEvalCorrection(const Position& pos, ScoreType evalScore, ScoreType trueScore)
{
    ...
    // material
    {
        ...
        matScore = static_cast<int16_t>((matScore * (EvalCorrectionBlendFactor - 2) + diff) / EvalCorrectionBlendFactor);
    }
    ...
    // pawn structure
    {
        ...
        pawnScore = static_cast<int16_t>((pawnScore * (EvalCorrectionBlendFactor - 2) + diff) / EvalCorrectionBlendFactor);
    }

    // PSQT
    constexpr int32_t PsqtErrorBlendFactor = 1024;
    for (const Color c : {Color::White, Color::Black})
    {
        const auto& side = pos.GetSide(c);
        for (const Piece p : {Piece::Knight, Piece::Bishop, Piece::Rook, Piece::Queen})
        {
            ...
            side.GetPieceBitBoard(p).Iterate([&](const Square& square) INLINE_LAMBDA
            {
                ...
                psqtErr = static_cast<int16_t>((psqtErr * (PsqtErrorBlendFactor - 2) + diff) / PsqtErrorBlendFactor);
            });
        }
        {
