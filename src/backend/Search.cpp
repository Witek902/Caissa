...
DEFINE_PARAM(EvalCorrectionMinorPiecesScale, 50, 1, 128);
...
void Search::Clear()
{
    for (const ThreadDataPtr& threadData : mThreadData)
    {
        ...
        memset(threadData->minorPiecesCorrection, 0, sizeof(threadData->minorPiecesCorrection));
...
ScoreType Search::ThreadData::GetEvalCorrection(const NodeInfo& node) const
{
    ...
    corr += EvalCorrectionMinorPiecesScale * minorPiecesCorrection[stm][node.position.GetMinorPiecesHash() % EvalCorrectionTableSize];
...
ScoreType Search::NegaMax(ThreadData& thread, NodeInfo* node, SearchContext& ctx) const
{
    ...
    if (!filteredSomeMove && !CheckStopCondition(thread, ctx, false))
    {
             ...
             (bestValue > unadjustedEval && bestMove.IsValid())))
        {
            ...
            AddToCorrHist(thread.minorPiecesCorrection[stm][position.GetMinorPiecesHash() % ThreadData::EvalCorrectionTableSize], bonus);
