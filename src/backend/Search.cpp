...
DEFINE_PARAM(EvalCorrectionMinorsScale, 48, 1, 128);
...
void Search::Clear()
{
    for (const ThreadDataPtr& threadData : mThreadData)
    {
        ...
        memset(threadData->minorsCorrection, 0, sizeof(threadData->minorsCorrection));
...
ScoreType Search::ThreadData::GetEvalCorrection(const NodeInfo& node) const
{
    ...
    corr += EvalCorrectionMinorsScale * minorsCorrection[stm][node.position.GetMinorsHash() % EvalCorrectionTableSize];
...
ScoreType Search::NegaMax(ThreadData& thread, NodeInfo* node, SearchContext& ctx) const
{
    ...
    if (!filteredSomeMove && !CheckStopCondition(thread, ctx, false))
    {
             ...
             (bestValue > unadjustedEval && bestMove.IsValid())))
        {
            ...
            AddToCorrHist(thread.minorsCorrection[stm][position.GetMinorsHash() % ThreadData::EvalCorrectionTableSize], bonus);
