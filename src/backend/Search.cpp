...
ScoreType Search::ThreadData::GetEvalCorrection(const Position& pos) const
{
    ...
    const int32_t pieceStructIndex = pos.GetPawnsHash() % PawnStructureCorrectionTableSize;
...
void Search::ThreadData::UpdateEvalCorrection(const Position& pos, ScoreType evalScore, ScoreType trueScore)
{
    ...
    // pieces structure
    {
        const int32_t index = (pos.GetHash() ^ pos.GetPawnsHash()) % PieceStructureCorrectionTableSize;
        int16_t& pieceScore = pieceStructureCorrection[index];
