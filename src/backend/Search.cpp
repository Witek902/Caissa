...
DEFINE_PARAM(LateMoveReductionScale_Quiet, 74);
DEFINE_PARAM(LateMoveReductionBias_Quiet, 58);
DEFINE_PARAM(LateMoveReductionScale_Noisy, 51);
DEFINE_PARAM(LateMoveReductionBias_Noisy, 47);
...
DEFINE_PARAM(ReductionStatOffset, 8840);
DEFINE_PARAM(ReductionStatDiv, 7470);
    ...
    while (movePicker.PickMove(*node, ctx.game, move, moveScore))
    {
            ...
            moveIndex > (1u + isPvNode))
        {
            ...
            if (move.IsQuiet())
            {
                ...
                if (moveScore >= MoveOrderer::KillerMoveBonus - 1) r -= 2;
            ...
            {
                r = GetNoisyDepthReduction(node->depth, moveIndex);
