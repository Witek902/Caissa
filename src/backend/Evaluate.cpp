...
ScoreType Evaluate(NodeInfo& node, AccumulatorCache& cache)
{
    ...
    // apply scaling based on game phase (0 - endgame, 36 - opening)
    const int32_t gamePhase = std::min(24,
        2 * whiteKnights + blackKnights + whiteBishops + blackBishops +
        3 * (whiteRooks + blackRooks) +
        4 * (whiteQueens + blackQueens));
    value = value * (110 + gamePhase) / 128;
