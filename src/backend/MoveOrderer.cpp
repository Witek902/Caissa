    ...
    const NodeCacheEntry* nodeCacheEntry) const
{
    ...
    for (uint32_t i = 0; i < moves.Size(); ++i)
    {
        ...
        if (move.IsCapture())
        {
            ...
            if ((uint32_t)attackingPiece < (uint32_t)capturedPiece || ((move.ToSquare().GetBitboard() & threats) == 0))
                score = WinningCaptureValue;
            else if (attackingPiece == capturedPiece || pos.StaticExchangeEvaluation(move))
