...
void MoveOrderer::UpdateCapturesHistory(const NodeInfo& node, const Move* moves, uint32_t numMoves, const Move bestMove)
{
    ...
    for (uint32_t i = 0; i < numMoves; ++i)
    {
        ...
        UpdateHistoryCounter(capturesHistory[color][threats.IsBitSet(to)][pieceIdx][capturedIdx][to], delta);
    }
}
    ...
    for (uint32_t i = 0; i < moves.Size(); ++i)
    {
        ...
        if (move.IsCapture())
        {
            ...
            // capture history
            {
                ...
                ASSERT(capturedIdx < 5);
                ASSERT(pieceIdx < 6);
                const int32_t historyScore = ((int32_t)capturesHistory[color][threats.IsBitSet(to)][pieceIdx][capturedIdx][to] - INT16_MIN) / 128;
